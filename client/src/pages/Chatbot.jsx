  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div className={`flex items-start space-x-2 max-w-xs lg:max-w-md ${isUser ? "flex-row-reverse space-x-reverse" : ""}`}>
        ...
        <div className={`rounded-2xl px-4 py-2 ${isUser ? "bg-blue-500 text-white" : "bg-gray-100 dark:bg-gray-800 dark:text-gray-100"}`}>
          {message.image && (
            <img
              src={message.image}
              alt="Uploaded"
...
const ChatBot = ({ user }) => {
    ...
    const fetchChatHistory = async () => {
      if (user?._id) {
        try {
          ...
          setMessages(
            data.length > 0
              ? data.map(msg => ({ ...msg, id: msg._id || Date.now().toString() }))
              : [
                  {
                    id: "1",
                    text: `നമസ്കാരം ${user?.name || "കർഷകാ"}! I am your Digital Krishi Officer. How can I help you today?`,
                    sender: "bot",
                    timestamp: new Date(),
                  },
                ]
  ...
  const saveMessageToBackend = async (message) => {
    if (!user?._id) return; // skip guests
    try {
  ...
  // Generate bot response from backend
  const fetchBotResponse = async (userMessage) => {
    try {
      ...
      const data = await res.json();
      return data.response || "I did not understand that. Can you elaborate?";
    } catch (err) {
  ...
  const handleSendMessage = async () => {
    ...
    const botText = user?._id ? await fetchBotResponse(inputText) : "I understand your question. Please give me more details.";
    const botMessage = {
  ...
  const handleVoiceRecord = () => {
    ...
    if (!isRecording) {
      setTimeout(() => {
        ...
        setInputText("My tomato plants have leaf spot disease...");
      }, 3000);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 pt-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-t-2xl border-b border-gray-200 p-6 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Digital Krishi Officer</h1>
          ...
          <div className="flex items-center space-x-2">
            <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
              <button
                ...
                className={`px-3 py-1 rounded-md text-xs font-medium ${selectedLanguage === "ml" ? "bg-white dark:bg-gray-600 text-gray-900 dark:text-white" : "text-gray-600 dark:text-gray-300"}`}
              >
                മലയാളം
              </button>
              <button
                onClick={() => setSelectedLanguage("en")}
                className={`px-3 py-1 rounded-md text-xs font-medium ${selectedLanguage === "en" ? "bg-white dark:bg-gray-600 text-gray-900 dark:text-white" : "text-gray-600 dark:text-gray-300"}`}
              >
                English
              </button>
            </div>
            <button onClick={() => setDarkMode(!darkMode)} className="p-2 rounded-lg text-gray-500 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-b-2xl border-t border-gray-200 p-4 flex items-center space-x-2">
          ...
          <button onClick={() => fileInputRef.current.click()} className="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-300">
            <Camera className="h-5 w-5" />
          </button>
          <button onClick={handleVoiceRecord} className={`p-2 ${isRecording ? "text-red-500" : "text-gray-500 dark:text-gray-300"}`}>
